...
const GameSection = ({ setSavedGameName, showNotification, showPopup }) => {
  ...
  useEffect(() => {
    ...
    const fetchGames = async () => {
      try {
        const url = new URL("/api/fetch_games", window.location.origin);
        url.searchParams.append("search", gameName);

        const res = await fetch(url);

        if (!res.ok) {
          throw new Error("Failed to fetch games from proxy");
        }
  ...
  return (
    <>
      <section
        ref={wrapperRef}
        className="bg-white m-3 mb-5 mt-5 md:ml-30 md:mr-30 md:mt-8 xl:ml-50 xl:mr-50 shadow-md rounded-4xl p-6 font-serif"
      >
        ...
        <form onSubmit={handleSubmit} className="mt-3">
          ...
          <div className="relative w-full mt-2 mb-4">
            ...
            {isFocused && showResults && results.length > 0 && (
              <ul className="absolute top-full left-0 z-10 w-full mt-1 bg-white border rounded-xl shadow-lg max-h-60 overflow-y-auto max-w-md">
                {results.map((game) => (
                  <li
                    key={game.id}
